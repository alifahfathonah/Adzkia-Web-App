...
include "include/koneksi.php";
  ...
  $ij=mysql_query("select * from hasil where id_siswa='$siswa' and id_jadwaltry='$jadwal' and bi_kosong!='' ");
...
if ($po>0) {
   echo "<script>
     alert('Maaf, Anda tidak bisa kembali ke menu soal ini.');
      document.location.href='?p=nilai.php&siswa=$siswa&jadwal=$jadwal'
      </script>";
}
else
{
...
<div class="col-md-3 box-right">
  ...
  <span>WAKTU : <font color="red" id="future_date"></font> 
    <?php
      $tt=mysql_query("select * from mulaibi where id_siswa='$siswa' and id_jadwaltry='$jadwal'");
      ...
      <script>
        ...
        $(document).ready(function(){
          countdowntimer(10, 'index.php?p=soalbi_proses.php&siswa=<?php echo $siswa; ?>&jadwal=<?php echo $jadwal;?>');
        });*/
      </script>
  </span>
</div>
...
<?php $t=mysql_query("select * from soalbaru where id_jadwaltry='$jadwal' and nomor>='14' and nomor<='18' order by id_soalbaru ASC");
...
<div class="col-md-3 box-sidebar">
  <form action="?p=soalbi_proses.php" method="post">
  ...
  <input type="hidden" name="siswa" value="<?php echo $siswa; ?>"/>
    <input type="hidden" name="jadwal" value="<?php echo $jadwal; ?>"/>
     ...
     <?php 
        ...
        echo "<b>Soal</b>"; 
       $l=mysql_query("select * from kunci k where id_jadwaltry='$jadwal' and status_soal='bi'");
       ...
       <input type="hidden" name="soal" value="<?php echo $u['id_soalbaru']; ?>"/>
           ...
           <td> <input type="hidden" class="minimal pilihan" value="<?php echo $u['nomor'];?>" name="nomor<?php echo $r ?>" ><?php echo $u['nomor'] ?></td> 
...
<script type="text/javascript">
  /*$(".button").click(function(){
    //alert('asdasd');
    if (!confirm("Anda yakin ingin mengakhiri tryout?\nHarap periksa kembali jawaban anda")){
      return false;
    }
  });*/
    $(document).ready(function() {
